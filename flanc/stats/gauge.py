...
#
# Modified by Rick Porter, Applied Communicatin Sciences, 2016
#
...
from dp import DP
from faucet_util import kill_on_exception
...
class GaugeFlowTableInfluxDBPoller(GaugeInfluxDBPoller):
    """Periodically dumps the current datapath flow table as a yaml object.
    ...
    matches all flows."""
    def __init__(self, dp, ryudp, logname):
        super(GaugeFlowTableInfluxDBPoller, self).__init__(dp, ryudp, logname)
        self.interval = self.dp.monitor_flow_table_interval
        self.logfile = self.dp.monitor_flow_table_file
    ...
    def update(self, rcv_time, msg):
        ...
        for stat in msg.body:
            flow_name = str(stat.cookie)

            flow_tags = dict(stat.match.items())
            for eth in ('eth_dst', 'eth_src'):
                if eth in flow_tags and not isinstance(flow_tags[eth],str):
                    flow_tags[eth] = '/'.join(list(flow_tags[eth]))

            flow_tags['dp_name'] = self.dp.name
            flow_tags['cookie'] = stat.cookie
            flow_tags['participant'] = stat.cookie >> 16
            flow_tags['priority'] = stat.priority
            flow_tags['table_id'] = stat.table_id

            for stat_name, stat_value in (
                ("packet_count", stat.packet_count),
                ("byte_count", stat.byte_count)):
                points.append({
                    "measurement": 'flow_' + stat_name,
                    "tags": flow_tags,
                    "time": int(rcv_time),
                    "fields": {"value": stat_value}})
        if not self.ship_points(points):
