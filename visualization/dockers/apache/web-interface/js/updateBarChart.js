...
function() {/*Own Rickshaw Series - Start*/
	...
	{
		console.log("gauges key: " + key);
	}

	gauges["F1"].redraw(523);
	gauges["F2"].redraw(543);
	gauges["F3"].redraw(520);
	gauges["F4"].redraw(520);

	gauges["B1"].redraw(523);
	gauges["B2"].redraw(543);
	gauges["B3"].redraw(520);
	gauges["B4"].redraw(520);
...
socket.on('message', function(message){
		...
		{
			dial = message.split("|")[1];
			value= message.split("|")[2];

		  if(dial == "1") { gauges["F1"].redraw(value); }
			if(dial == "2") { gauges["F2"].redraw(value); }
			if(dial == "3") { gauges["F3"].redraw(value); }
			if(dial == "4") { gauges["F4"].redraw(value); }
		}
		else if(type == "dialb")
    {
      dial = message.split("|")[1];
      ...
      if(dial == "1") { gauges["B1"].redraw(value); }
      if(dial == "2") { gauges["B2"].redraw(value); }
      if(dial == "3") { gauges["B3"].redraw(value); }
      if(dial == "4") { gauges["B4"].redraw(value); }
    }
}) ;
